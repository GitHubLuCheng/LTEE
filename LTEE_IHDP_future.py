...
import cfr.cfr_net_future as cfr
...
tf.app.flags.DEFINE_integer('rep_weight_decay', 1,
...
x_DIM = 25
t0 = 50  ##we can observe data up until t=t0, predict the effect at t=T
T_ls=range(55,105,5)
tf.set_random_seed(1)
for T in T_ls:
    print T
    result_file = open('data/IHDP/future/' + str(T) + '.txt', 'w')
    for j in range(1, 11):
        ...
        ys = np.concatenate((out_treat[:, 1:(t0+1)], out_treat[:, T].reshape(N, 1)), axis=1)
        ...
        CFR = cfr.cfr_net(x, t, y_, p, FLAGS, r_alpha, r_lambda, do_in, do_out, dims, test)
        ...
        X_train, X_test, y_train, y_test, t_train, t_test = train_test_split(matrix_rep, ys, ts, test_size=0.2,random_state=1)
